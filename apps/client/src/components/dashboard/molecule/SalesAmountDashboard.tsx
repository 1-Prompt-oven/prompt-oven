...
import { StatisticHistoryResponse } from '@/types/dashboard/DashboardType';
...
function SalesDashboard({ selectedPeriod, selectedUnit }: PeriodUnitType) {
  ...
  const [data, setData] = useState([] as StatisticHistoryResponse[]);
  ...
  useEffect(() => {
    ...
    const groupedData = data.map((item) => item.sales);
    const labels = data.map((item) => item.targetDate);
